...
function RootLayoutNav() {
  const { userProfile, theme, gender } = useApp();
  ...
  useEffect(() => {
    ...
    supabase.auth.getSession().then(({ data: { session } }) => {
      ...
      const isGenderSelection = segments[0] === 'gender-selection';

      if (!session && !inAuthGroup) {
        router.replace('/(auth)/login');
      } else if (session) {
        if (!gender && !isGenderSelection) {
          router.replace('/gender-selection');
        } else if (gender && (inAuthGroup || isGenderSelection)) {
          router.replace('/(tabs)');
        }
      }
    });
  ...
  }, [fontsLoaded, gender]); // added gender to dependencies

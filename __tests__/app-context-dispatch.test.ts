...
function dispatchSyncUserData(
 ...
 isAdmin: args.username === 'postly' || args.username === 'admin',
 };
}
...
describe('AppContext dispatch — setUser (updateProfile)', () => {
 ...
 expect(onlyName.userProfile.username).toBe('postly_user');
 expect(onlyName.userProfile.bio).toBe('Hello, I am using Postly');
 ...
 expect(initial.userProfile.name).toBe('Postly User'); // original untouched
 });
});
...
describe('AppContext dispatch — logout transition', () => {
 ...
 expect(loggedOut.userProfile.name).toBe('Postly User');
 expect(loggedOut.userProfile.username).toBe('postly_user');
 expect(loggedOut.userProfile.bio).toBe('Hello, I am using Postly');

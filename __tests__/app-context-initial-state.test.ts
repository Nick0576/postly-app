...
function buildInitialState(savedBlockedUsers: string[] | null = null) {
  ...
  if (savedBlockedUsers && ls) {
    ls.setItem('postly-blocked-users', JSON.stringify(savedBlockedUsers));
    const saved = ls.getItem('postly-blocked-users');
    if (saved) {
      try {
    ...
    ls.removeItem('postly-blocked-users');
  }
...
describe('AppContext — default user profile', () => {
  ...
  it('uses the Postly placeholder display name and handle', () => {
    const state = buildInitialState();
    expect(state.userProfile.name).toBe('Postly User');
    expect(state.userProfile.username).toBe('postly_user');
  });

  it('uses the canonical welcome bio', () => {
    const state = buildInitialState();
    expect(state.userProfile.bio).toBe('Hello, I am using Postly');
  });
...
describe('AppContext — module wiring', () => {
  it('the AppContext source file exists and exports a provider', () => {
    ...
    expect(contents).toContain("name: 'Postly User'");
  });
  ...
  it('keeps the initial state defaults in lockstep with the source file', () => {
    ...
    expect(src).toContain("username: 'postly_user'");
    expect(src).toContain("bio: 'Hello, I am using Postly'");

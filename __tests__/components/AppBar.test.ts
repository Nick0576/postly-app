...
describe('AppBar – title rendering', () => {
  it('returns the trimmed title when non-empty', () => {
    expect(formatTitle('Postly Social')).toBe('Postly Social');
  });
  ...
  it('falls back to "Postly Social" when title is empty', () => {
    expect(formatTitle('')).toBe('Postly Social');
    expect(formatTitle('   ')).toBe('Postly Social');
  });
});
...
describe('AppBar – combined root-screen scenario', () => {
  it('root screen: no back, toggle visible, dark theme label', () => {
    ...
    const title = formatTitle('Postly Social');

    expect(back).toBe(false);
    expect(toggle).toBe(true);
    expect(label).toBe('🌙');
    expect(title).toBe('Postly Social');
  });
});

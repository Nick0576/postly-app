...
function makeFeedPost(overrides: Partial<Post> = {}): Post {
  return {
    ...
    username: 'noor',
    avatar: null,
    content: 'Just joined Postly! #hello',
    media_type: 'text',
    likes: 0,
    reposts: 0,
    replies: 0,
...
function makePostRow(overrides: {
  ...
  return {
    ...
    content: overrides.content ?? 'Just joined Postly! #hello',
...
describe('E2E — complete signup flow', () => {
  ...
  it('rejects signup when the chosen username is already taken', async () => {
    ...
    testHooks.setHandler('profiles', () => ({ data: { username: 'postly' }, error: null }));

    const isTaken = await checkUsernameExists('postly');
...
describe('E2E — post create flow', () => {
  it('publishes a text post end-to-end and returns the populated Post', async () => {
    ...
    expect(published!.content).toBe('Just joined Postly! #hello');

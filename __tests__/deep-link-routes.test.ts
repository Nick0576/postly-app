...
// Verifies that `postly://` custom-scheme URLs and `https://postly.social/`
...
const SCHEME = 'postly';
const UNIVERSAL_HOST = 'postly.social';
...
const parseDeepLink = (url: string): ParsedDeepLink => {
  ...
  // Accept both the custom scheme `postly://...` and universal `https://postly.social/...`
  ...
  // For `postly://post/abc`, the URL API yields host="post", pathname="/abc".
  // For `https://postly.social/post/abc`, host="postly.social", pathname="/post/abc".
...
describe('deep-link route parsing — post/:id', () => {
  it('parses postly://post/<uuid> into /post/[id] with the id', () => {
    const result = parseDeepLink('postly://post/8f1c2b9a-1234-4abc-9def-0123456789ab');
  ...
  it('parses https://postly.social/post/<numeric-id> into /post/[id]', () => {
    const result = parseDeepLink('https://postly.social/post/42');
    expect(result).toEqual({
      route: '/post/[id]',
      params: { id: '42' },
    });
...
describe('deep-link route parsing — profile/:username', () => {
  it('parses postly://user/<username> into /user/[username]', () => {
    const result = parseDeepLink('postly://user/postly_dev');
    expect(result).toEqual({
      route: '/user/[username]',
      params: { username: 'postly_dev' },
    });
  });

  it('parses https://postly.social/user/<username> into /user/[username]', () => {
    const result = parseDeepLink('https://postly.social/user/sara.postly');
    expect(result).toEqual({
      route: '/user/[username]',
      params: { username: 'sara.postly' },
    });
  });
});

...
describe('useApp (AppContext) — provider wrapper', () => {
  beforeEach(() => {
    ...
    window.localStorage.removeItem('postly-blocked-users');
  });
  ...
  it('exposes the expected default state shape to consumers', () => {
    ...
    try {
      ...
      expect(ctx!.userProfile.name).toBe('Postly User');
      expect(ctx!.userProfile.username).toBe('postly_user');
      // Default theme.
